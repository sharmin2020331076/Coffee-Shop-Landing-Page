...
export function Cappuccino(){
    ...
    return(
        <div className="w-full min-h-screen grid lg:grid-cols-2 grid-cols-1 gap-4 lg:gap-8 items-center">
                <div className="p-14">
                    <div ref={textRef} className={`bg-yellow-100 rounded-3xl p-6 sm:p-10 lg:p-14 space-y-4 sm:space-y-6 order-2 lg:order-1 flex flex-col justify-center hover:shadow-2xl hover:scale-105 transition-all ease-out cursor-pointer ${textVisible ? 'animate-fade-in-left' : 'opacity-0'}`}>
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-coffee font-sans">Our Story</h1>
                     <p className="text-sm sm:text-base lg:text-lg text-coffee font-sans leading-relaxed">Coffee Break is your gateway to a world of rich, aromatic experiences crafted for true coffee lovers. As an online store, it curates an exceptional range of specialty coffees sourced ethically from diverse regions across the globe. Whether you're drawn to the bold depth of medium-dark roast single origin beans or the playful notes of flavored espresso, Coffee Break promises something to delight every palate...... </p>
                    <Button text="Learn More" height="40px" width="110px" variant="gradient"/>
                </div>
                </div>

                <div ref={imageRef} className={`relative w-full flex justify-center items-center order-1 lg:order-2 p-4 sm:p-8 lg:p-0 overflow-hidden ${imageVisible ? 'animate-fade-in-right' : 'opacity-0'}`}>
